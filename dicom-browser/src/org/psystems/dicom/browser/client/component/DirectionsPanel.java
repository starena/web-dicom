...
public class DirectionsPanel extends Composite {
    ...
    SearchPanel spanel;
    protected static String modality;
	...
	Browser.browserService.getDeviceModalities(new AsyncCallback<HashMap<String, String>>() {
		...
		lbManufacturerModelName.addItem("OT (прочее)", "MODALITY=OT");
		for (String mod : result.keySet()) {
		    lbManufacturerModelName.addItem(result.get(mod) + " (" + mod + ")", "MODALITY=" + mod);
		}
	...
	// for (ConfigDevice configDevice : Config.getDevices()) {
	// lbManufacturerModelName.addItem(configDevice.getDescription(),
	// configDevice.getName());
	// }

	Browser.browserService.getAllDevices(new AsyncCallback<ArrayList<ConfigDeviceProxy>>() {
		...
		lbManufacturerModelName.addItem("", "");
		for (ConfigDeviceProxy dev : result) {
		    lbManufacturerModelName.addItem(dev.getDescription(), dev.getName());
		}
	    }
	...
	// lbManufacturerModelName.addItem("Маммограф (LORAD AFFINITY)",
	// "LORAD AFFINITY");
	// lbManufacturerModelName.addItem("Эндоскоп (FUJINON EG WR5)",
	// "FUJINON EG WR5");
	// lbManufacturerModelName.addItem("Рентген (CLINOMAT)", "CLINOMAT");
	// lbManufacturerModelName.addItem("Рентген (РДК 50/6)", "РДК 50/6");
	// lbManufacturerModelName.addItem("Рентген (DUODiagnost)",
	// "DUODiagnost");
	//
	// lbManufacturerModelName.addItem("УЗИ-1 (SonoScape) ДП 523 каб.",
	// "SonoScape-1");
	// lbManufacturerModelName.addItem("УЗИ-2 (SonoScape)", "SonoScape-2");
	// lbManufacturerModelName.addItem("УЗИ-3 (SonoScape)", "SonoScape-3");
	// lbManufacturerModelName.addItem("УЗИ (VIVID) ДП 527 каб.",
	// "VIVID 3");
	// lbManufacturerModelName.addItem("VOLUSON 730 BT04 EXPERT",
	// "УЗИ (VOLUSON)");
	// lbManufacturerModelName.addItem("УЗИ (Acuson Sequoia) каб.408",
	// "Acuson Sequoia");
	//
	...
	lbManufacturerModelName.addChangeHandler(new ChangeHandler() {
		...
		if(manufacturerModelName.startsWith("MODALITY=")) {
//		    System.out.println("!!! manufacturerModelName="+manufacturerModelName);
		    modality = manufacturerModelName.replaceFirst("MODALITY=", "");
		    manufacturerModelName = null;
		}
    ...
    // counter++;
    //
    // Application.addToWorkStatusMsg(" Поиск продолжается " + counter * 2 +
    // " сек.");
    //
    // }
    // };
    ...
    //
    // Application.browserService.getDirections(query, new
    // AsyncCallback<ArrayList<DirectionProxy>>() {
    //
    // @Override
    // public void onFailure(Throwable caught) {
    // transactionFinished();
