...
public class DirectionsPanel extends Composite {
	...
	public DirectionsPanel(Dicom_browser application) {
		...
		studyDateBoxEnd.addValueChangeHandler(new ValueChangeHandler<Date>() {
			...
			@Override
			public void onValueChange(ValueChangeEvent<Date> event) {
				dateEnd = Utils.dateFormatSql.format(event.getValue()) + " 23:59:59";
				// System.out.println("!!!!val="+dateBegin);
				queryDirections();
			}
		});
		...
		DOM
				.setStyleAttribute(resultPanel.getElement(), "background",
						"#E9EDF5");
...
//		resultPanel.clear();
		...
		Application.browserService.getDirections(query,
				new AsyncCallback<ArrayList<DirectionProxy>>() {
					...
					public void onSuccess(ArrayList<DirectionProxy> result) {
						...
						Label l = new Label("Количество Направлений: "
								+ result.size());
	...
	protected void showNotFound() {
		...
		// emptyStr.setStyleName("DicomItemValue");
		emptyStr
				.setHTML("Ничего не найдено... Попробуйте выбрать другие даты...");
