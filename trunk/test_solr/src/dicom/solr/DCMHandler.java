 ...
 *
 **/
public class DCMHandler extends DefaultHandler {
  ...
  DCMHandler(SolrServer server, OSMFactFinder factFinder) {
    this(server, factFinder, DEFAULT_BUFFER_SIZE);
  }

  public DCMHandler(SolrServer server, OSMFactFinder finder, int bufferCount) {
    this.server = server;
